...
from lande.fermi.likelihood.diffuse import merge_diffuse
...
def get_insignificant_diffuse(roi, allowed_fraction=0.01, quiet=False):
    insignificant_list = []
    ...
    for source in get_background(roi):
        ...
        if not quiet: print 'Source %s predicts %s%% of total counts' % (source,fraction),
        if fraction < allowed_fraction:
            insignificant_list.append(source)
        else:
            if not quiet: print '... keep source.'
...
def merge_insignificant_diffuse(roi, *args, **kwargs):
    insignificant = get_insignificant_diffuse(roi, *args, **kwargs)

    if len(insignificant) == 0:
        # nothing to merge
        pass
    if len(insignificant) == 1:
        # only one source, no need to merge it
        pass
    else:
        for source in insignificant:
            roi.del_source(source)
        merged = merge_diffuse(insignificant)
