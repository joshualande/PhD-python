...
def smooth_plots(pipeline, roi, name, hypothesis, size):
    ...
    roi.plot_source(filename='%s/source_%s.png' % (pipeline.dirdict['plots'], extra), 
                    title=title,
                    **smooth_kwargs)

    title = 'Sources Map for %s (%s)' % (name,hypothesis);print title
    roi.plot_sources(filename='%s/sources_%s.png' % (pipeline.dirdict['plots'], extra), 
                     title=title,
                     **smooth_kwargs)
    ...
    ROISourceBandPlotter(roi, bin_edges=[1e3,1e4,10**5.5], 
                         title=title,
                         **smooth_kwargs).show(filename='%s/band_source_%s.png' % (pipeline.dirdict['plots'],extra))

    title = 'Band Sources Map for %s (%s)' % (name,hypothesis);print title
    ROISourcesBandPlotter(roi, bin_edges=[1e3,1e4,10**5.5],
                         title=title,
                          **smooth_kwargs).show(filename='%s/band_sources_%s.png' % (pipeline.dirdict['plots'],extra))
